...
5. 配置worker的变量
    - 增加`BOT_TOKEN`变量，数值为从步骤1中获得的token
    - 增加`BOT_SECRET`变量，数值为从步骤2中获得的secret
    - 增加`ADMIN_UID`变量，数值为从步骤3中获得的用户id
    - 增加`GROUP_CHAT_ID`变量，数值为`-12345678`群组id 并开启话题功能
6. 创建一个空间名称为`nfd`的kv数据库，在设置 -> 绑定 -> 添加绑定 `KV 命名空间`：nfd -> nfd
7. 点击`编辑代码`，复制[这个文件](./私聊%2B群组.js)到编辑器中
8. 通过打开[此链接](https://telegram.wuyangdaily.workers.dev/registerWebhook)注册websoket
9. 通过打开[此链接](https://telegram.wuyangdaily.workers.dev/setCommands)设置命令菜单
